    ...
    for _, emp in df_filtered.iterrows():
        ...
        with cols[col_idx]:
            with st.container(border=True):
                ...
                # 3. Modal (st.dialog)
                if st.button("Ver Detalle Completo", key=f"btn_{cedula}"):
                    with st.dialog("Detalle del Empleado"):
                        st.subheader(nombre)
...
def show_nomina(df_nomina, df_empleados_master):
    ...
    total_real = df_nomina['SALARIO_REAL'].sum()
    total_bruto = df_nomina['SALARIO_BRUTO'].sum()
    total_empr = df_nomina['CONTRIBUCION_EMPR'].sum()
    total_empl = df_nomina['CONTRIBUCION_EMPL'].sum()
...
def main():
    ...
    st.image("https.sicet.com.co/wp-content/uploads/2021/08/logo-sicet-azul.png", width=250)
